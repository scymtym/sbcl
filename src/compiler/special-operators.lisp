...
(defun make-special-operator-expansion (name lambda-list components
  ...
  (when documentationp
    (setf documentation (car documentation))
  ...
  (with-unique-names (cont-var whole-var)
    (binding* ((component-forms
      ...
      `(progn
         ...
         #-sb-xc-host
         ;; The guard function is a closure, which can't have its
         ;; lambda-list changed independently of other closures based
         ;; on the same simple-fun. So change it in the ir1-translator
         ;; since the actual lambda-list is not terribly useful.
         (when nil ; TODO later
           (install-guard-function
            ',name '(:special ,name) ,(or #!+sb-doc documentation)))
