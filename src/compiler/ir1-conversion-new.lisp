...
;;;
;;; The hash-tables used to hold global namespace info must be
...
(defun ir1-toplevel/new (instead recurse form path for-value)
  (declare (list path))
  (let* ((*current-path* path)
    ...
    (let* ((forms (if for-value `(,form) `(,form nil)))
           (res (ir1-convert-lambda-body/new
                 instead recurse forms ()
...
(defun walk-forms-for-ir1-conversion (function form)
  (walk-forms
   'list #|TODO list is hack to make hook processing work |# (add-hook-processing function) form ; TODO only if requested
   (named-lambda classify-variable-for-ir1-conversion (form)
     (classify-variable-form form *lexenv*))
   (named-lambda classify-application-for-ir1-conversion (form)
     (classify-application-form form *lexenv*))
   (named-lambda expand-macro-for-ir1-conversion (kind expander form)
     (sb!c:expand-macro kind expander form *lexenv*))))

(declaim (special *walk-mode*))
(defvar *stuff*)

(defun add-node-arguments (function) ; TODO unused
  (flet ((add-node-arguments-thunk (start next result &rest args)
           (let ((*stuff* (list start next result)))
             (apply function args))))
    #'add-node-arguments-thunk))

(defmacro with-node-arguments ((context instead recurse) &body body) ; TODO remove context argument
  (binding* (((instead-var instead-form) ; TODO ensure-list
              (if (consp instead)
                  (values-list instead)
                  (values instead instead)))
             ((recurse-var recurse-form)
              (if (consp recurse)
                  (values-list recurse)
                  (values recurse recurse))))
   (with-unique-names (instead-name recurse-name)
     `(flet ((,instead-name (start next result &rest args) ; TODO dx
               (let ((*stuff* (list start next result)))
                 (apply ,instead-form args)))
             (,recurse-name (start next result &rest args)
               (let ((*walk-mode* :old))
                 (print (list* ',context :recurse args))
                 (terpri))
               (let ((*stuff* (list start next result)))
                 (prog1
                     (apply ,recurse-form args)
                   (let ((*walk-mode* :old))
                     (print (list* :done ',context :recurse args))
                     (terpri))))))
        (let ((,instead-var #',instead-name)
              (,recurse-var #',recurse-name))
          ,@body)))))

(defun ir1-convert-walk-function (instead recurse form info name &rest components)
  (let ((*walk-mode* :old))
    (print (list 'ir1-convert-walk-function form info name components))
    (terpri))
  (binding* ((*current-path* (ensure-source-path form))
             ((start next result) (values-list *stuff*)))
    (with-node-arguments (ir1-convert-walk-function (instead1 instead) (recurse1 recurse))
      (etypecase info
        (variable-info
         (ir1-convert-var/new
          start next result instead1 recurse1 form))
        (self-evaluating-info
         (reference-constant/new start next result form))
        (leaf-info
         (reference-leaf start next result form))
        (special-operator-info
         (let ((converter (find-ir1-converter name))) ; TODO find-ir1-converter-or-lose
           (let ((*walk-mode* :old))
             (unless converter
               (bug "~@<IR1 converter for ~S not found.~@:>" name)))
           (apply converter instead1 recurse1 start next result
                  form components)))
        (named-application-info
         ...
         (ecase (getf components :where)
           ...
           (:lexical
            (let ((function (getf components :definition #+no :function))
                  (arguments (getf components :arguments)))
              (etypecase function
                (functional
                 (ir1-convert-local-combination/new
                  start next result instead1 recurse1 function arguments))
                (global-var
                 (ir1-convert-srctran/new
                  start next result instead1 recurse1 function form)))))))
        (lambda-application-info
         (funcall instead :form `(%funcall ,@form)))
        (compiler-macro-info
         (error "not implemented yet"))
        (macroid-info
         (funcall recurse))             ; look at IR1-CONVERT-FUNCTOID
        ))))
...
(macrolet (;; Bind *COMPILER-ERROR-BAILOUT* to a function that throws
           ...
           (ir1-error-bailout ((#+no instead #+no recurse start next result form)
                               &body body)
             (declare (ignore #+no recurse start next result form))
             (with-unique-names (skip condition)
               `(block ,skip
                  (let ((,condition (catch 'ir1-error-abort
                    ...
                    #+no (funcall ,instead ,start ,next ,result
                             :form (make-compiler-error-form
                                    ,condition ,form))))))) ; TODO just use the restart?
  ...
  ;; if necessary.
  (defun reference-constant/new (start next result value)
...
(declaim (ftype (function (ctran ctran (or lvar null) function function symbol) (values &optional))
                ir1-convert-var/new))
(defun ir1-convert-var/new (start next result instead recurse name)
  ...
  (let ((var (or (lexenv-find name vars) (find-free-var name))))
    (if (and (global-var-p var) (not (always-boundp name)))
        ...
        (funcall instead start next result
                 :form (if (eq (global-var-kind var) :global)
        ...
        (etypecase var
          ...
          (heap-alien-info
           (funcall instead start next result :form `(%heap-alien ',var))))))
  (values))
...
#+TODO(declaim (ftype (function (ctran ctran (or lvar null) function function list) (values &optinoal))
                ir1-convert-progn-body/new))
(defun ir1-convert-progn-body/new (start next result instead recurse body)
  (declare (ignore recurse))
  (if (endp body)
      (reference-constant/new start next result nil)
      #+TODO (loop for (form . rest) on body)
      (let ((this-start start)
            (forms body))
        (loop
          (let ((form (car forms)))
            (when (endp (cdr forms))
              (funcall instead this-start next result :form form)
              (return))
            (let ((this-ctran (make-ctran)))
              (funcall instead this-start this-ctran nil :form form)
...
(defun ir1-convert-combination/new (start next result instead recurse
  ...
  (let ((ctran (make-ctran))
        (fun-lvar (make-lvar)))
    (funcall instead start ctran fun-lvar
             :form `(the (or function symbol) ,function))
    (ir1-convert-combination-args/new
     ctran next result instead recurse fun-lvar arguments)))
...
(defun ir1-convert-combination-args/new (start next result instead recurse
  ...
  (let ((node (make-combination fun-lvar)))
    ...
    (collect ((arg-lvars))
      (let ((this-start start))
        (dolist (arg arguments) ; TODO simplify with loop/collect?
          (let ((this-ctran (make-ctran))
                (this-lvar (make-lvar node)))
            (funcall instead this-start this-ctran this-lvar
                     :form arg)
            (setq this-start this-ctran)
            (arg-lvars this-lvar)))
...
;;; optimization.
(declaim (ftype (sfunction (ctran ctran (or null lvar) function function functional list) t)
                ir1-convert-local-combination/new))
(defun ir1-convert-local-combination/new (start next result instead recurse
                                          functional arguments)
  (assure-functional-live-p functional)
  (ir1-convert-combination/new
   start next result instead recurse
   (maybe-reanalyze-functional functional) arguments))

;;;; PROCESS-DECLS
