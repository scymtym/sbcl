...
(defun ir1-convert-aux-bindings/new (start next result instead recurse
                                     body aux-vars aux-vals
                                     post-binding-lexenv)
  (declare (type ctran start next)
           (type (or lvar null) result)
           (type list body aux-vars aux-vals))
  (if (null aux-vars)
      ...
      (let ((ctran (make-ctran))
            (fun-lvar (make-lvar))
            (fun (ir1-convert-lambda-body/new
                  instead recurse body (list (first aux-vars))
                  :aux-vars (rest aux-vars)
                  :aux-vals (rest aux-vals)
                  :post-binding-lexenv post-binding-lexenv
                  :debug-name (debug-name '&aux-bindings aux-vars))))
        (reference-leaf start ctran fun-lvar fun)
        (ir1-convert-combination-args/new
         ctran next result instead recurse
         fun-lvar (list (first aux-vals)))))
  (values))
...
(defun ir1-convert-special-bindings/new (start next result instead recurse
                                         body aux-vars aux-vals svars post-binding-lexenv)
  (declare (type ctran start next)
           (type (or lvar null) result)
           (type list body aux-vars aux-vals svars))
  (cond
    ((null svars)
     (ir1-convert-aux-bindings/new
      start next result instead recurse
      body aux-vars aux-vals post-binding-lexenv))
    (t
     ...
     (let ((cleanup (make-cleanup :kind :special-bind))
       ...
       (funcall instead start bind-ctran nil
                :form `(%special-bind ',(lambda-var-specvar var) ,var))
       (setf (cleanup-mess-up cleanup) (ctran-use bind-ctran))
       (let ((*lexenv* (make-lexenv :cleanup cleanup)))
         (funcall instead bind-ctran cleanup-ctran nil
                  :form '(%cleanup-point))
         (ir1-convert-special-bindings/new
          cleanup-ctran next result instead recurse
          body aux-vars aux-vals (rest svars) post-binding-lexenv)))))
  (values))
...
(defun ir1-convert-lambda-body/new (instead recurse
                                    body vars
                                    &key
                                    aux-vars
                                    aux-vals
                                    ...
                                    post-binding-lexenv
                                    system-lambda)
  (declare (type list body vars aux-vars aux-vals))
      ...
      (let ((*lexenv* (make-lexenv :vars (new-venv)
        ...
        (with-component-last-block (*current-component*
          ...
          (let ((prebind-ctran (make-ctran))
            ...
            (progn #+no with-node-arguments #+no (ir1-convert-lambda-body instead recurse)
              (ir1-convert-special-bindings/new
               postbind-ctran result-ctran result-lvar instead recurse
               body aux-vars aux-vals (svars) post-binding-lexenv))))))
...
;;; by default forms.
(defun convert-optional-entry/new (instead recurse fun vars vals defaults name)
  (declare (type clambda fun)
           (type list vars vals defaults))
  (let* ((fvars (reverse vars))
         ...
         (fun (collect ((default-bindings)
                ...
                (let ((bindings (default-bindings))
                  ...
                  (ir1-convert-lambda-body/new
                   instead recurse
                   (if bindings
                       `((let (,@bindings) ,call))
                       `(,call))
                   arg-vars
                   ;; FIXME: Would be nice to share these names
                   ;; instead of consing up several identical ones. Oh
                   ;; well.
                   :debug-name (debug-name '&optional-processor name)
...
(defun generate-optional-default-entry/new (instead recurse
  ...
  (declare (type optional-dispatch res)
           (type list default-vars default-vals entry-vars entry-vals vars body
                      aux-vars aux-vals))
  (let* ((arg (first vars))
         ...
         (force (or force (not (sb!xc:constantp (arg-info-default info)))))
         (ep (if supplied-p
                 (ir1-convert-hairy-args/new
                  instead recurse
                  res
    ...
    (let ((name (or debug-name source-name)))
      (if (or force
          ...
          (convert-optional-entry/new
           instead recurse ep default-vars default-vals
          ...
          (let* ((default `',(constant-form-value default))
            ...
            (delay
             (register-entry-point
              (convert-optional-entry/new
               instead recurse
               (force ep) default-vars default-vals defaults name)
              res)))))))
...
(defun convert-more-entry/new (instead recurse
                               res entry-vars entry-vals rest morep keys name)
  (declare (type optional-dispatch res)
           (type list entry-vars entry-vals keys))
  (collect ((arg-vars)
    ...
    (let* ((n-context (sb!xc:gensym "N-CONTEXT-"))
      ...
      (let ((ep (ir1-convert-lambda-body/new
                 instead recurse
                 `((let ,(temps)
                     ,@(body)
...
(defun ir1-convert-more/new (instead recurse
                             res default-vars default-vals entry-vars entry-vals
  ...
  (declare (type optional-dispatch res)
           (type list default-vars default-vals entry-vars entry-vals keys body
                      aux-vars aux-vals))
  (collect ((main-vars (reverse default-vars))
    ...
    (let* ((main-entry (ir1-convert-lambda-body/new
                        instead recurse
                        body (main-vars)
           ...
           (last-entry (convert-optional-entry/new
                        instead recurse
                        main-entry default-vars (main-vals) () name)))
      (setf (optional-dispatch-main-entry res)
            (register-entry-point main-entry res))
      (convert-more-entry/new
       instead recurse
       res entry-vars entry-vals rest more-context keys name)

      (push (register-entry-point
             (if supplied-p-p
                 (convert-optional-entry/new
                  instead recurse
                  last-entry entry-vars entry-vals () name)
                last-entry)
             res)
...
(defun ir1-convert-hairy-args/new (instead recurse
                                   res default-vars default-vals
                                   entry-vars entry-vals
  ...
  (cond ((not vars)
         (if (optional-dispatch-keyp res)
             ...
             (let* ((name (or debug-name source-name))
               ...
               (push (if supplied-p-p
                         (register-entry-point
                          (convert-optional-entry/new
                           instead recurse fun entry-vars entry-vals () name)
                          res)
                         fun)
                     (optional-dispatch-entry-points res))
               fun)))
        ...
        (t
         (let* ((arg (first vars))
           ...
           (ecase kind
             (:optional
              (let ((ep (generate-optional-default-entry/new
                         instead recurse
                         res default-vars default-vals
                ...
                ep))
             (:rest
              (ir1-convert-more/new     ; TODO make a local function
               instead recurse
               res default-vars default-vals
               entry-vars entry-vals
...
(defun ir1-convert-hairy-lambda/new (instead recurse
                                     body vars keyp allowp aux-vars aux-vals
  ...
  (let ((res (make-optional-dispatch :arglist vars
    ...
    (ir1-convert-hairy-args/new
     instead recurse
     res () () () () vars nil body aux-vars aux-vals
...
(defun ir1-convert-lambda/new (instead recurse
  ...
  (binding* (((vars keyp allow-other-keys aux-vars aux-vals)
             ...
             ((*lexenv* result-type post-binding-lexenv)
              (process-decls declarations (append aux-vars vars) nil
                             :binding-form-p t))
             (body (if (eq result-type *wild-type*)
                       body
                       `((the ,(type-specifier result-type) (progn ,@body)))))
             (res (cond ((or (find-if #'lambda-var-arg-info vars) keyp)
                         (ir1-convert-hairy-lambda/new
                          instead recurse
                          body vars keyp
                          allow-other-keys
                          aux-vars aux-vals
                        ...
                        (t
                         (ir1-convert-lambda-body/new
                          ...
                          :debug-name debug-name
                          :system-lambda system-lambda)))))
    (setf (functional-inline-expansion res) body
          (functional-arg-documentation res) (cadr form)
          (functional-documentation res) documentation)
      ...
      (let ((name (cadr thing))
        ...
        (if (and name (legal-fun-name-p name))
            ...
            (convert lambda-expression
                     :maybe-add-debug-catch t
                     :debug-name (or name (name-lambdalike thing))))))
     ((lambda-with-lexenv)
      (convert thing :source-name source-name :debug-name debug-name)))))

...
(defun ir1-convert-inline-lambda/new (instead recurse fun
                                      &key
                                      (source-name '.anonymous.)
                                      debug-name
                                      system-lambda)
  ...
  (destructuring-bind (decls macros symbol-macros &rest body)
    ...
    (let* ((*lexenv* (make-lexenv
           ...
           (clambda (ir1-convert-lambda/new ; TODO just call instead?
                     instead recurse
                     `(lambda ,@body) () () () body ; TODO how does this work?
                     :source-name source-name
                     :debug-name debug-name
                     :system-lambda system-lambda)))
