    ...
    (dx-flet ((interpret-directives (stream)
      ...
      (if per-line-p
          (pprint-logical-block
              ...
              (stream arg :per-line-prefix prefix :suffix suffix :circularity (not atsignp))
            (let ((*logical-block-popper* (lambda () (pprint-pop))))
              (interpret-directives stream)))
          (pprint-logical-block (stream arg :prefix prefix :suffix suffix :circularity (not atsignp))
            (let ((*logical-block-popper* (lambda () (pprint-pop))))
              (interpret-directives stream))))))
  (if atsignp nil args))

