...
(defun real-specializer-type-specifier/specializer
    (proto-generic-function proto-method specializer)
  (declare (ignore generic-function proto-method))
         ...
         (class-name-type-specifier (name)
           (case (info :type :kind name)
             ...
             ((:instance :forthcoming-defclass-type)
              ;; CLOS classes are here but are too expensive to check
              ;; (as opposed to STRUCTURE-CLASS and SYSTEM-CLASS)
              ;; which have their own methods and thus are not handled
              ;; here.
              nil)
             (t
              (warn-find name)))))
  ...
  (defun real-specializer-type-specifier/t
    ...
    (handler-case
      ...
      (error (condition)
        ...
        ;; does not conform to the respective extended specializer
        ;; syntax.
        (warn-parse specializer condition)
        nil)))
      ...
      ((delegations ()
         `(typecase specializer
            ,@(mapcar
               ...
               '(; specializer
                 ; ^ apparently not needed during bootstrapping
                 symbol t class-eq-specializer eql-specializer
